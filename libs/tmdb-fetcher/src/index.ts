...
export function createTmdbClient(config: TmdbConfig): HttpClient {
...
export class TmdbService {
  ...
  async findTmdbMovieId(movie: MovieRecord): Promise<number> {
    ...
    for (const candidate of basicCandidates) {
      const movie = await this.searchMovie(candidate)
      if (movie)
        return movie.id
    }
  ...
  async findTmdbTvShowId(tvShow: TvShowRecord): Promise<number> {
    ...
    for (const candidate of basicCandidates) {
      const tvShow = await this.searchTvShow(candidate)
      if (tvShow)
        return tvShow.id
    }
  ...
  async searchMovie({ title, year }: SearchMovieCandidate): Promise<MovieSearchResult | null> {
    const searchResults = await this.searchMovies(title, year)
    const match = findMovieMatch(searchResults, title, year)
    return match ?? null
  }

  async searchTvShow({ title }: SearchTvShowCandidate): Promise<TvShowSearchResult | null> {
    const searchResults = await this.searchTvShows(title)
  ...
  async getMovieDetails(id: number): Promise<TmdbMovieDetails> {
    ...
    const movieDetails = await this.httpClient.get(`/movie/${id}?${query}`) as MovieDetailsResponse
  ...
  async getTvShowDetails(id: number): Promise<TmdbTvShowDetails> {
    const query = new URLSearchParams({ language: 'cs' })
    const tvShowDetails = await this.httpClient.get(`/tv/${id}?${query}`) as TvShowDetailsResponse
    return {
      id,
  ...
  private async searchMovies(title: string, year: number): Promise<MovieSearchResult[]> {
    ...
    const response = await this.httpClient.get(`/search/movie?${query}`) as MovieSearchResponse
    return response.results
  }

  private async searchTvShows(title: string): Promise<TvShowSearchResult[]> {
    const query = new URLSearchParams({
      query: title,
      language: 'cs',
    })

    const response = await this.httpClient.get(`/search/tv?${query}`) as TvShowSearchResponse
    return response.results
  }
}
...
export function createTmdbService(config: TmdbConfig): TmdbService {
  const httpClient = createTmdbClient(config)
  return new TmdbService(httpClient)
}
